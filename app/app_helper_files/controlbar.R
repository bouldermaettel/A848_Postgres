controlbar <-
  ...
  conditionalPanel('input.tabs == `admin`',
  controlbarMenu(
  id = "menu",
  controlbarItem(
  NULL,
 ...
 h4('Postgresql database'),
  ...
  br(),
   h4('Export'),
    ...
    selectizeInput("postgres_tables", label = "Choose postgres table to for data extraction", choices=NULL,  multiple =F, options = NULL),
        appButton(inputId = "execute", label = 'get_data', icon = icon("save")),
  selectizeInput("columns_admin", label = "Choose columns to be shown", choices=NULL,  multiple =T, options = NULL),
  br(),
