...
format_datasets <- function(data) {
  return(data %>% datatable( options = list(searching = T, pageLength=20, lengthMenu = c(10, 20, 30, 50, 100, 200),
             ...
             filter = list( position = 'top', clear = FALSE), fillContainer = FALSE))
}

get_rows <- function(rows_selected, rows_all) {
...
make_sheet <- function(sheet_name, data, rows, wb) {
  openxlsx::addWorksheet(wb[['duplicates']], sheetName = sheet_name)
  openxlsx::writeData(wb[['duplicates']], sheet = sheet_name, x = data[rows,], startCol = 1, startRow = 1)
}

assign_data <- function(source, data_hist, data_new, data_all, rows, columns) {
      if (source == 'historic'){
      return(data_hist[rows,columns])
    } else if (source == 'new') {
      return(data_new[rows,columns])
    } else if (source == 'all' | source == 'historic & new') {
      return(data_all[rows,columns])
    }
}
...
# paste(sample(samp,8),collapse="")
    ...
    #   data$orig <- data$hist[data_rows,input$columns]
    # } else if (input$data_source == 'new') {
    #   data$orig <- data$new[data_rows,input$columns]
    # } else if (input$data_source == 'all' | input$data_source == 'historic & new') {
    #   data$orig <- data$all[data_rows,input$columns]
    # }
